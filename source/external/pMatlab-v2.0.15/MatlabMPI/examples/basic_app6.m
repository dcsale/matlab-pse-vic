...
% figure window.
%
% This example builds on basic_app4.m and basic_app5.m. 
% It keeps track of each of the jobs that must be completed, 
% and keeps track of whether each of the worker processes are busy or not. 
...
%
% This example builds on basic_app5.m by adding a job queue 
% for each of the worker processes. This eliminates the 
% potential idle time that a worker process may encounter 
% of the time between when it submitted a result and when 
% it receives its next job message. 
%
...
%
%   eval( MPI_Run('basic_app6',4,{}) );
%
% Or, to run a different machine type:
%
%   eval( MPI_Run('basic_app6',4,{'machine1' 'machine2' 'machine3' 'machine4'}) );
%
% Output will be piped into 4 files:
%
%   MatMPI/basic_app6.0.out
%   MatMPI/basic_app6.1.out
%   MatMPI/basic_app6.2.out
%   MatMPI/basic_app6.3.out
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Dr. Albert Reuther
...
% The worker job queue depth must be a positive integer
worker_job_queue_depth = 2;
...
if (my_rank == leader)
    ...
    while ~done
        % Deal input data to everyone else (excluding leader).
        if (sendCounter <= N2) & (sum(workers_busy) < (worker_job_queue_depth * num_workers))
            % determine which queues are not full    
            queue_deficit = workers_busy - worker_job_queue_depth;
            [enqueued, worker_index] = sort(queue_deficit);
            
            % send to the worker with the emptiest queue
            dest = worker_index(1);
            ...
            workers_busy(dest) = workers_busy(dest) + 1;
    ...
    while ~done
        ...
        if ~isempty(message_ranks)
            ...
            if in_tag ~= end_tag
                ...
                % Add a delay so that the job doesn't complete so quickly
                pause(0.5);
