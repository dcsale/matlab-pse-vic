...
%  if (isunix) host = getenv('HOST');          end
  ...
  q = strrep(' '' ',' ','');
  qq = '"';
  ssh_response='./MatMPI/remote.out';
...
%                                  qq 'cd /d ' pwd_pc ' & start /b ' dos_matlab_file qq nl];
            ...
            unix_launch_i_m = [remote_launch machine remote_flags ...
                               q 'cd ' pwd_unix '; /bin/sh ./' unix_matlab_file '  ' q ' 2>>' ssh_response ' &' nl];
...
%                                 qq 'cd /d ' pwd_pc ' & start /b ' dos_matlab_file qq nl];
  ...
  unix_launch
  if ispc
     %% Identify Dos_Commands.{hostname}.{ID}.bat files
     strBeg = regexp(unix_launch,'Dos_Comm\w*');
     strEnd = regexp(unix_launch,'.bat');
  end
...
if (ispc)
   % The following batch file incurrs scalability issue
   %     [status,result] = system([launch_file]);
   %     [status,result] = dos([launch_file]);
   % Instead of using DOS BAT file, use the system() call to launch MATLAB
   %     (This part can be moved to MatMPI_Commands.m)
   % Check if the launch file has multiple lines of batch file commands
   mlines = length(strBeg);
   for i=1:mlines
       fileName = ['.\MatMPI\' unix_launch(strBeg(i):strEnd(i)+3)];
       fid = fopen(fileName);
       % Assuming 11 fields to read based on the dos batch file &
       % skip copy lines
       D = textscan(fid, '%s %s %s %s %s %s %s %s %s %s %s','commentStyle', 'copy');
       % celldisp(D);
       nparts = length (D);
       % Allows multiple MATLAB instances in each batch file
       mcomms = length(D{1});
       for jj = 1:mcomms
           cmd='';
           for kk=1:nparts; cmd=[cmd ' ' char(D{kk}{jj})]; end
           % disp(cmd);
           [~, results] = system(cmd);
       end
       fclose(fid);
