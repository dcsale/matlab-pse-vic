...
dbstop in VortexParticle at 95
...
% Initialize part 1
% ========================================================================%
[SIM, ENV, PART, MESH] = VortexParticle_Init(inputFile);
...
switch SIM.example
        ...
        % @@@@@@@@@@@@@@@@@@@@@@@@,,@@@@@@@@@@@@@@@@@@@@@@@@@    
           ...
           wf         = init_field(SIM, MESH);                  % init vorticity field
               [xp, PART] = init_particle_pos(PART, MESH, wf);      % init particle locations (remeshing function)
               wp         = interp_M2P(PART, MESH, xp, wf);         % init particle strengths (remeshing function)
               ap         = wp .* prod(MESH.dx);                    % init particle circulations (remeshing function)
           wf_p       = interp_P2M(PART, MESH, xp, wp);         % interpolate particle vorticity to field (P2M)
           uf         = PoissonSolve3D(wf, MESH, SIM);        	% solve Poisson eqn for velocity
           up         = interp_M2P(PART, MESH, xp, uf);         % interpolate velocity field to particles (M2P)
           ...
           plot_particles(xp, ap, MESH, 'Particle Circulation')
        ...
        %% =======================================================================%
        % init vorticity field
        % ========================================================================%
        wf = init_field(SIM, MESH);
        
        %% =======================================================================%
        % solve Poisson eqn for velocity field
        % ========================================================================%
%         profile on
        if SIM.solve_vel == 0
            stream = PoissonSolve3D(wf, MESH, SIM);
        else
            vel    = PoissonSolve3D(wf, MESH, SIM);
        end
%         profile viewer
%         profile off
...
%            MESH       = init_mesh(SIM, MESH);                   % init mesh
%            wf         = init_field(SIM, MESH);                  % init vorticity field
%            [xp, PART] = init_particle_pos(PART, MESH, wf);      % init particle locations
% %            [wp, ap]   = interp_M2P(PART, MESH, xp, wf);         % init particle strengths
%            vel = PoissonSolve3D(wf, MESH, SIM);                 % solve Poisson eqn for velocity
%            
% %            plot_field(xp, wp, wf, MESH, 'Vorticity Field')
%            
%            clear MESH wf vel xp ap
%            
%         end
...
%         plot_field(     wf, MESH, 'Vorticity Field')
        ...
        %% =======================================================================%
        % init particle positions and strengths (remesh operator M2P)
        % ========================================================================%
        [xp, wp, ap] = interpM2P(PART, MESH, wf);
